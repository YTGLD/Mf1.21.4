...
public class meye extends ItemTir {
    public meye(Settings settings) {
        super(settings);
    }

    @Override
    public void tick(ItemStack stack, SlotReference reference) {
        Vec3d vec3d = reference.entity().getPos();
        int r = 10;
        List<LivingEntity> list = reference.entity().getEntityWorld().getEntitiesByClass(LivingEntity.class,new Box(vec3d.x + r,vec3d.y + r,vec3d.z + r,vec3d.x - r,vec3d.y - r,vec3d.z - r), EntityPredicates.EXCEPT_SPECTATOR);
        for (LivingEntity livingEntity : list){
            if (livingEntity != reference.entity()){
