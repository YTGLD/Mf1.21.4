...
public class death_penalty extends TheNecoraIC {
    ...
    public death_penalty(Settings settings) {
        super(settings);
    }
    ...
    public static void hurts(LivingEntity living , DamageSource source){
        if (source.getSource() instanceof PlayerEntity player){
            if (HasCurio.has(init.death_penalty,player)){
                if (!player.getItemCooldownManager().isCoolingDown(init.death_penalty.getDefaultStack())) {
                    ...
                    player.getItemCooldownManager().set(init.death_penalty.getDefaultStack(), 20);
