...
public class Handler {
    ...
    private static void addSquareColor(VertexConsumer vertexConsumer, MatrixStack poseStack, Vec3d up1, Vec3d up2, Vec3d down1, Vec3d down2, float alpha, float r, float g, float b) {
        // 添加四个顶点来绘制一个矩形
        vertexConsumer.vertex(poseStack.peek().getPositionMatrix(), (float)up1.x, (float)up1.y, (float)up1.z)
                .color(r* 255, g* 255, b* 255, (int)(alpha * 255))
                .light(240, 240)
                .normal(0, 0, 1);
    ...
    public static void renderLineColor(MatrixStack poseStack, VertexConsumerProvider bufferSource, Vec3d start, Vec3d end, float a, RenderLayer renderType, float radius, float r, float g, float b) {
        VertexConsumer vertexConsumer = bufferSource.getBuffer(renderType);
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            addSquareColor(vertexConsumer, poseStack, up1, up2, down1, down2, a,r,g,b);
        }
    }
