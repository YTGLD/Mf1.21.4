...
public class line extends TameableZombie {
    ...
    public void tick() {
        ...
        if (this.getOwner()!=null
            ...
            for (LivingEntity living : entities) {
                if (this.target != null
                        && living ==(this.target)
                        && player.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE)!=null)
                {
                    cloudTime = 20;
                    float dam = (float) player.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE).getValue();
                    dam/=20;
                    if (this.target.age % 5 == 0) {
                        this.target.timeUntilRegen = 0;
                        this.target.serverDamage(living.getDamageSources().mobAttack(this), dam);
                    }
                }
            }
        }
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        line line = InItEntity.Line.create(world, SpawnReason.BREEDING);
        if (line != null) {
