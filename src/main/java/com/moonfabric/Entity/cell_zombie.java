...
public class cell_zombie  extends TameableZombie {
    ...
    private Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> calcificationMultimap(LivingEntity livingEntity){
       Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> modifierMultimap = HashMultimap.create();
        if (HasCurio.has(init.cell, livingEntity) && HasCurio.has(init.cell_calcification, livingEntity)) {
            modifierMultimap.put(EntityAttributes.ARMOR,new EntityAttributeModifier(Identifier.of("base_attack_damage"+"cell_armor"), livingEntity.getAttributeValue(EntityAttributes.ARMOR) / 2, EntityAttributeModifier.Operation.ADD_VALUE));
            modifierMultimap.put(EntityAttributes.MAX_HEALTH, new EntityAttributeModifier(Identifier.of("base_attack_damage"+"cell_health"), livingEntity.getAttributeValue(EntityAttributes.MAX_HEALTH) / 2, EntityAttributeModifier.Operation.ADD_VALUE));
        }
    ...
    private Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> modifierMultimap(LivingEntity livingEntity){
       Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> modifierMultimap = HashMultimap.create();
        if (HasCurio.has(init.cell, livingEntity) && HasCurio.has(init.adrenaline, livingEntity)) {
            modifierMultimap.put(EntityAttributes.ATTACK_DAMAGE, new EntityAttributeModifier(Identifier.of("base_attack_damage"+"cell_damage"), livingEntity.getAttributeValue(EntityAttributes.ATTACK_DAMAGE), EntityAttributeModifier.Operation.ADD_VALUE));
            modifierMultimap.put(EntityAttributes.MOVEMENT_SPEED, new EntityAttributeModifier(Identifier.of("base_attack_damage"+"cell_speed"), livingEntity.getAttributeValue(EntityAttributes.MOVEMENT_SPEED), EntityAttributeModifier.Operation.ADD_VALUE));
        }
        return modifierMultimap;
    }
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        cell_zombie line = InItEntity.cell_zombie.create(world, SpawnReason.BREEDING);
        if (line != null) {
