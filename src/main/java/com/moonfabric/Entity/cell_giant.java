...
public class cell_giant extends TameableZombie {
    ...
    public static DefaultAttributeContainer.Builder addAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.MAX_HEALTH, 100).add(EntityAttributes.MOVEMENT_SPEED, 0.30000001192092896).add(EntityAttributes.KNOCKBACK_RESISTANCE, 1.0).add(EntityAttributes.ATTACK_KNOCKBACK, 1.5).add(EntityAttributes.ATTACK_DAMAGE, 10);
    }
    ...
    private Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> AttributeModifier(cell_giant cellGiant, LivingEntity living){
        ...
        if (cellGiant.getCommandTags().contains(HasCurio.Bone_Giant)) {
            modifierMultimap.put(EntityAttributes.ARMOR,new EntityAttributeModifier(Identifier.of("base_attack_damage"+"cell_armor"), living.getAttributeValue(EntityAttributes.ARMOR)* 0.7, EntityAttributeModifier.Operation.ADD_VALUE));
        }
