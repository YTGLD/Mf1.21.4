...
public class nightmare_base_insight_insane extends com.moonfabric.item.Ms.SNightmare{
    public nightmare_base_insight_insane(Settings settings) {
        super(settings);
    }
    ...
    public static void LivingDeathEvents(DamageSource  source, LivingEntity entity){
        if (source.getSource() instanceof PlayerEntity player) {
            if (HasCurio.has(init.nightmare_base_insight_insane, player)) {
                ...
                player.getItemCooldownManager().set(init.nightmare_base_insight_insane.getDefaultStack(),200);
            }
        }
    }
    public static void damage(DamageSource  source , CallbackInfoReturnable<Float> cir){
        if (source.getAttacker() instanceof PlayerEntity player) {
            if (HasCurio.has(init.nightmare_base_insight_insane, player)) {
                if (player.getItemCooldownManager().isCoolingDown(init.nightmare_base_insight_insane.getDefaultStack())){
                    cir.setReturnValue(cir.getReturnValue()*2.5f);
                    player.getItemCooldownManager().set(init.nightmare_base_insight_insane.getDefaultStack(),0);
                }
            }
        }
    }
