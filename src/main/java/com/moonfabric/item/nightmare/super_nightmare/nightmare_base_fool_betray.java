...
public class nightmare_base_fool_betray extends com.moonfabric.item.Ms.SNightmare{
    ...
    public nightmare_base_fool_betray(Settings settings) {
        super(settings);
    }
                ...
                for (MobEntity mob : entities) {
                    if (mob instanceof Ownable ownableEntity) {
                        if (ownableEntity.getOwner()!=null&& !(ownableEntity.getOwner() ==(player))) {
                            ownableEntity.getOwner().serverDamage(ownableEntity.getOwner().getDamageSources().dryOut(),
                    ...
                    reference.entity().getWorld().playSound(null, reference.entity().getX(), reference.entity().getY(), reference.entity().getZ(), SoundEvents.ENTITY_ELDER_GUARDIAN_CURSE, SoundCategory.AMBIENT, 1, 1);
                    player.getItemCooldownManager().set(this.getDefaultStack(), 100 + entities.size() * 100);
                }
            }
        }
    }
