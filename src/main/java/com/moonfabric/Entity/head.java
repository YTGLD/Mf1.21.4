...
public class head extends TameableEntity {
                ...
                if (this.getOwner() != null) {
                    if (!(entity == (this.getOwner())) && this.getOwner() instanceof PlayerEntity player) {
                        ...
                        if (!entitys.getNamespace().equals(MoonFabricMod.MODID)) {
                           if (onAttack <= 0) {
                               if (player.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE)!=null) {
                                   entity.timeUntilRegen = 0;
                                   entity.serverDamage(this.getOwner().getDamageSources().dryOut(), (float) (2 + (player.getMaxHealth() / 10)+(player.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE).getValue() / 10f)));
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        head line = InItEntity.head.create(world, SpawnReason.BREEDING);
        if (line != null) {
