...
public class dna extends AccessoryItem {
    public dna(Settings settings) {
        super(settings.maxCount(1).component(Data.BUNDLE_CONTENTS,
    ...
    public void tick(ItemStack stack, SlotReference reference) {
        if (reference.entity() instanceof PlayerEntity player){
            player.getAttributes().addTemporaryModifiers(this.getModifiers(stack));
            BundleContentsComponent bundlecontents = stack.get(Data.BUNDLE_CONTENTS);
            if (bundlecontents != null) {
                bundlecontents.iterate().forEach((itemStack -> {
    ...
    public void onUnequip(ItemStack stack, SlotReference reference) {
        reference.entity().getAttributes().addTemporaryModifiers(this.getModifiers(stack));

    }

    public Multimap<RegistryEntry<EntityAttribute>, EntityAttributeModifier> getModifiers(ItemStack stack) {
        ...
        if (bundlecontents != null) {
            bundlecontents.iterate().forEach((itemStack -> {
                if (itemStack.getItem() == (DNAItems.atp_height)) {
                    int count = itemStack.getCount();
                    int a = count / 4;
                    multimap.put(EntityAttributes.MAX_HEALTH, new EntityAttributeModifier(
                ...
                if (itemStack.getItem() == (DNAItems.cell_oxygen)) {
                    ...
                    count /= 100f;
                    count *= 0.5F;
                    multimap.put(EntityAttributes.MOVEMENT_SPEED, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                ...
                if (itemStack.getItem() == (DNAItems.cell_in_water)) {
                    float count = itemStack.getCount();
                    count /= 100f;
                    multimap.put(EntityAttributes.WATER_MOVEMENT_EFFICIENCY, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_VALUE));
                }
                ...
                if (itemStack.getItem() == (DNAItems.cell_in_air)) {
                    float count = itemStack.getCount();
                    count /= 100f;
                    multimap.put(EntityAttributes.JUMP_STRENGTH, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                if (itemStack.getItem() == (DNAItems.cell_ground)) {
                    ...
                    count /= 100f;
                    count *= 2F;
                    multimap.put(EntityAttributes.BLOCK_BREAK_SPEED, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                ...
                if (itemStack.getItem() == (DNAItems.cell_bone_add)) {
                    float count = itemStack.getCount();
                    count /= 4f;
                    multimap.put(EntityAttributes.ARMOR, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_VALUE));
                }
                if (itemStack.getItem() == (DNAItems.cell_sense)) {
                    float count = itemStack.getCount();
                    count /= 100f;
                    multimap.put(EntityAttributes.OXYGEN_BONUS, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_VALUE));

                    multimap.put(EntityAttributes.SUBMERGED_MINING_SPEED, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count*10,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                if (itemStack.getItem() == (DNAItems.cell_synthesis)) {
                    float count = itemStack.getCount();
                    count /= 100f;
                    multimap.put(EntityAttributes.ATTACK_SPEED, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                if (itemStack.getItem() == (DNAItems.cell_putrefactive)) {
                    float count = itemStack.getCount();
                    count /= 100f;
                    multimap.put(EntityAttributes.BURNING_TIME, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            -count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                if (itemStack.getItem() == (DNAItems.cell_dna_suppression)) {
                    float count = itemStack.getCount();
                    count /= 100;
                    multimap.put(EntityAttributes.EXPLOSION_KNOCKBACK_RESISTANCE, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
                ...
                if (itemStack.getItem() == (DNAItems.cell_chromosome)) {
                    float count = itemStack.getCount();
                    count /= 10;
                    multimap.put(EntityAttributes.SAFE_FALL_DISTANCE, new EntityAttributeModifier(
                            Identifier.of("base_attack_damage"+this.getTranslationKey()),
                            count,
                            EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
                }
    ...
    private static final int ITEM_BAR_COLOR = MathHelper.hsvToRgb(0.1F, 0.8F, 0.5F);
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (bundlecontents != null) {
            bundlecontents.iterate().forEach((itemStack -> {
                tooltip.add(Text.translatable(itemStack.getItem().getTranslationKey()).append("："+ itemStack.getCount()).setStyle(Style.EMPTY.withColor(TextColor.fromRgb(0XFFDB7093))));
            }));
        }
    }
}
